...
import {
  extractCrossProjectBatch,
  fetchCrossProjectBatches,
  fetchBatch,
  approveBatchTask,
  rejectBatchTask,
  deleteBatch,
  setBatchTaskTargetProject,
} from "../../api/pm";
...
interface CrossProjectAISynthModalProps {
  isOpen: boolean;
  onClose: () => void;
}

export function CrossProjectAISynthModal({ isOpen, onClose }: CrossProjectAISynthModalProps) {
  ...
  const [setAllProjectId, setSetAllProjectId] = useState<string>("");
  ...
  const loadBatches = useCallback(async () => {
    try {
      const res = await fetchCrossProjectBatches();
      setBatches(res.data);
    } catch {
      /* ignore */
    }
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setView("grid");
      setError(null);
      // Make sure projects list is fresh for the per-task pickers
      fetchProjects();
    }
  }, [isOpen, loadBatches, fetchProjects]);
  ...
  const handleExtract = async () => {
    ...
    setError(null);
    try {
      const batch = await extractCrossProjectBatch(text || undefined, file || undefined);
      ...
      setError(message || "Extraction failed. Try again.");
    } finally {
  ...
  const handleSetTargetProject = async (taskId: string, targetProjectId: string) => {
    if (!activeBatch) return;
    try {
      await setBatchTaskTargetProject(activeBatch.id, taskId, targetProjectId);
      await refreshActiveBatch();
    } catch (err) {
      const message = (err as { error?: string })?.error;
      setError(message || "Could not set target project.");
    }
  };

  const handleSetAll = async () => {
    if (!activeBatch || !setAllProjectId) return;
    const pending = activeBatch.tasks?.filter((t) => t.status === "pending") ?? [];
  ...
  const handleApprove = async (taskId: string) => {
    ...
    try {
      ...
      await refreshActiveBatch();
      loadBatches();
      // Refresh projects so dashboard counts are correct
      fetchProjects();
    } catch (err) {
      const message = (err as { error?: string })?.error;
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  >
                    {view === "grid"
                      ? "AI Synth · Cross-project"
                      : view === "new"
                      ? "New Cross-project Synth"
                      : "Batch Review"}
                  </h2>
                  <p className="text-[11px]" style={{ color: "var(--color-pm-text-muted)" }}>
                    Extract tasks across your whole project list
                  </p>
                </div>
              </div>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5">
                ...
                (batches.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-12">
                    ...
                    <p
                      ...
                      style={{ color: "var(--color-pm-text-primary)" }}
                    >
                      No cross-project batches yet
                    </p>
                    <p
                      className="text-[12px] mb-4"
                      style={{ color: "var(--color-pm-text-muted)" }}
                    >
                      Paste text or upload a document. AI will extract tasks and
                      suggest a project for each.
                    </p>
                    <button
                      onClick={() => setView("new")}
                    ...
                    {batches.map((b) => {
                      ...
                      return (
                        <motion.div
                          key={b.id}
                          whileHover={{ y: -1 }}
                          className="group relative text-left rounded-xl p-4 cursor-pointer"
                          style={{
                            backgroundColor: "var(--color-pm-bg-primary)",
                            boxShadow: "var(--pm-shadow-card)",
                            border: "1px solid var(--color-pm-border-subtle)",
                          }}
                        ...
                        >
                          ...
                          <span
                            className="inline-block rounded-full px-2 py-0.5 text-[9px] font-semibold mb-2"
                            style={{
                              backgroundColor: "rgba(214,104,83,0.12)",
                              color: "#D66853",
                            }}
                          >
                            Cross-project
                          ...
                          {b.status === "pending_review" && b.total_proposed > 0 && (
                            <div className="flex items-center gap-2 mb-2">
                              <div
                                ...
                                style={{ backgroundColor: "var(--color-pm-bg-hover)" }}
                              >
                                <div
                                  className="h-full rounded-full"
                                  style={{
                                    width: `${
                                      ((b.total_approved + b.total_rejected) /
                                        b.total_proposed) *
                                      100
                                    }%`,
                          ...
                          >
                            {b.total_proposed > 0 && (
                              <>
                                {b.total_approved > 0 && (
                                  <span className="text-[#3D8B40]">
                                    {b.total_approved} approved
                                  </span>
                                )}
                                {b.total_rejected > 0 && (
                                  <>
              ...
              {view === "new" && (
                <div className="space-y-4">
                  <p
                    ...
                    style={{ color: "var(--color-pm-text-secondary)" }}
                  >
                    Paste text or upload a file. AI will extract actionable tasks and
                    suggest a target project for each one. You&rsquo;ll review and assign
                    ...
                    {file ? (
                      ...
                      <>
                        ...
                        <span
                          ...
                          style={{ color: "var(--color-pm-text-muted)" }}
                        >
                          Drop file or click (.txt, .pdf, .docx, .eml)
                        </span>
                      </>
                    )}
                    <input
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 rounded-lg py-2.5 text-[13px] font-semibold text-white disabled:opacity-50"
                    style={{ backgroundColor: "#D66853" }}
                  >
              ...
              {view === "detail" && activeBatch && (
                <div className="space-y-3">
                  ...
                  >
                    <summary
                      className="px-4 py-2.5 text-[12px] font-medium cursor-pointer"
                      style={{ color: "var(--color-pm-text-muted)" }}
                    >
                      Source: {activeBatch.source_filename || "Pasted text"} (
                      {activeBatch.total_proposed} tasks extracted)
                    </summary>
                    <div
                      className="px-4 pb-3 text-[12px] max-h-32 overflow-y-auto"
                      style={{ color: "var(--color-pm-text-secondary)" }}
                    >
                  ...
                  {/* Set all to */}
                  {activeBatch.tasks?.some((t) => t.status === "pending") && (
                    <div
                    ...
                    >
                      ...
                      <select
                        value={setAllProjectId}
                        onChange={(e) => setSetAllProjectId(e.target.value)}
                      ...
                      <button
                        ...
                        disabled={!setAllProjectId}
                        className="rounded-md px-3 py-1 text-[11px] font-semibold text-white disabled:opacity-50"
                        style={{ backgroundColor: "#D66853" }}
                      >
                        Apply
                      </button>
                  ...
                  {activeBatch.tasks?.map((t) => (
                    <CrossProjectBatchTaskCard
                      key={t.id}
                      task={t}
                      projects={projects}
                      onApprove={handleApprove}
                      onReject={handleReject}
                      onSetTargetProject={handleSetTargetProject}
                    />
                  ))}
                </div>
              )}
...
function CrossProjectBatchTaskCard({
  task,
  projects,
  onApprove,
  onReject,
  ...
  task: PmAiSynthBatchTask;
  projects: { id: string; name: string; status: string; icon: string; color: string }[];
  onApprove: (id: string) => Promise<void>;
  onReject: (id: string) => Promise<void>;
  onSetTargetProject: (id: string, targetProjectId: string) => Promise<void>;
}) {
  ...
  const targetProject = task.target_project_id
    ? projects.find((p) => p.id === task.target_project_id)
    : null;
  const TargetIcon = targetProject
    ? ((LucideIcons as unknown as Record<string, React.ComponentType<{ className?: string; strokeWidth?: number; style?: React.CSSProperties }>>)[
        targetProject.icon.charAt(0).toUpperCase() + targetProject.icon.slice(1)
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between mb-2">
        ...
        {task.status !== "pending" && (
          <span
            className="flex-shrink-0 flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-semibold"
      ...
      {/* Project picker (cross-project mode) */}
      {task.status === "pending" && (
        <div className="flex items-center gap-2 mb-2">
          {TargetIcon && targetProject && (
            <TargetIcon
          ...
          <select
            value={task.target_project_id ?? ""}
            onChange={(e) => onSetTargetProject(task.id, e.target.value)}
            className="flex-1 rounded-md px-2 py-1 text-[12px] outline-none"
            style={{
              backgroundColor: "var(--color-pm-bg-secondary)",
              color: task.target_project_id
                ? "var(--color-pm-text-primary)"
                : "var(--color-pm-text-muted)",
              border: `1px solid ${
                task.target_project_id ? "var(--color-pm-border)" : "rgba(196,51,51,0.3)"
              }`,
            }}
          >
      ...
      {task.status === "pending" && (
        <div className="flex gap-2 mt-2">
          <button
            ...
            disabled={loading !== null || !task.target_project_id}
            title={!task.target_project_id ? "Assign a project first" : undefined}
            className="flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-[12px] font-semibold text-white disabled:opacity-50 disabled:cursor-not-allowed"
          ...
          <button
            ...
            style={{ color: "#C43333", border: "1px solid rgba(196,51,51,0.3)" }}
          >
