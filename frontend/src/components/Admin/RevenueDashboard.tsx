...
function MRRTrendChart({ orgs }: { orgs: AdminOrganization[] }) {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-2xl p-6">
      <p className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-4">MRR Trend (6 months)</p>
...
export default function RevenueDashboard() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8 space-y-6">
      <h1 className="text-xl font-semibold text-[#1A1D23]">Revenue</h1>

      {/* Four metric cards */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="bg-white border border-gray-200 rounded-2xl p-5 text-center">
          <DollarSign className="h-5 w-5 text-emerald-500 mx-auto mb-2" />
          <p className="text-2xl font-semibold text-[#1A1D23]">${mrr.toLocaleString()}</p>
          <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mt-1">MRR</p>
          <p className="text-xs text-gray-400 mt-0.5">{activeOrgs.length} active</p>
        </div>

        <div className="bg-white border border-gray-200 rounded-2xl p-5 text-center">
          <Users className="h-5 w-5 text-blue-500 mx-auto mb-2" />
          <p className="text-2xl font-semibold text-[#1A1D23]">{trialOrgs.length}</p>
          <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mt-1">Trial Pipeline</p>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-5 text-center">
          ...
          <p className={`text-2xl font-semibold ${criticalOrgs.length > 0 ? "text-red-600" : "text-[#1A1D23]"}`}>
            {criticalOrgs.length}
          </p>
          <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mt-1">Churn Risk</p>
          <p className="text-xs text-gray-400 mt-0.5">
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-5 text-center">
          ...
          <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mt-1">NRR Proxy</p>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-gray-100">
                <th className="px-5 py-3 text-xs font-semibold uppercase tracking-wider text-gray-400">Practice</th>
                <th className="px-5 py-3 text-xs font-semibold uppercase tracking-wider text-gray-400">Monthly</th>
                <th className="px-5 py-3 text-xs font-semibold uppercase tracking-wider text-gray-400">Months</th>
                <th className="px-5 py-3 text-xs font-semibold uppercase tracking-wider text-gray-400">Health</th>
              ...
              {tableOrgs.map((org) => {
                ...
                return (
                  <tr key={org.id} className="border-b border-gray-50 last:border-0">
                    ...
                    <td className="px-5 py-3 text-sm text-[#1A1D23] tabular-nums">${orgMonthlyRate(org).toLocaleString()}</td>
