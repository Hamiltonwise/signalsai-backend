...
export async function up(knex: Knex): Promise<void> {
  ...
  await knex.schema
    ...
    .createTable("ai_command_batches", (t) => {
  ...
  await knex.schema
    .withSchema("website_builder")
    .createTable("ai_command_recommendations", (t) => {
      t.uuid("id").primary().defaultTo(knex.raw("gen_random_uuid()"));
      t.uuid("batch_id")
        .notNullable()
        .references("id")
        .inTable("website_builder.ai_command_batches")
        .onDelete("CASCADE");
  ...
  await knex.schema
    .withSchema("website_builder")
    .alterTable("ai_command_recommendations", (t) => {
...
export async function down(knex: Knex): Promise<void> {
  await knex.schema
    .withSchema("website_builder")
    .dropTableIfExists("ai_command_recommendations");
  await knex.schema
    .withSchema("website_builder")
    .dropTableIfExists("ai_command_batches");
}
