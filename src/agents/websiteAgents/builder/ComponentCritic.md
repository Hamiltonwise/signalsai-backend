...
10. **No non-background inline styles.** Scan the output for `style="..."` attributes. `background`, `background-image`, `background-color`, `background-position`, `background-size`, and `background-repeat` declarations are ALLOWED (templates legitimately ship hero / section backgrounds this way — preserve them). Any OTHER inline style declaration (`opacity`, `color`, `width`, `position`, `font-*`, etc.) is a fail with reason `INLINE_STYLE_USED`. Example fail: `style="opacity:0.9"`. Example pass: `style="background-image: linear-gradient(...), url(https://...);"`.
