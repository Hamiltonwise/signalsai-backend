...
export function siteQaBlockedTemplate(ctx: TemplateContext): NarratorOutput {
  ...
  const finding = `Your update to ${pagePath} almost shipped with ${defectCount} phrase${
    defectCount === 1 ? "" : "s"
  } every agency uses. Patients who have been burned by marketing spot that in seconds. We caught it before it went live.`;
